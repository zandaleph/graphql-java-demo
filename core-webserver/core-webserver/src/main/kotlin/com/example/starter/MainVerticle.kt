...
import com.example.starter.auth.DbSessionStore
import com.example.starter.auth.ExampleAuthenticationProvider
import com.example.starter.auth.SessionFlushingFormLoginHandler
...
class MainVerticle : CoroutineVerticle() {
    ...
    override suspend fun start() {
        ...
        val store = DbSessionStore.create(vertx, component.sessionFactory())
        val sessionHandler = SessionHandler.create(store).setLazySession(true)
        ...
        router.route("/login")
            ...
            .handler(
                SessionFlushingFormLoginHandler.create(sessionHandler, authProvider)
                    .setDirectLoggedInOKURL("/graphiql/index.html")
            )
